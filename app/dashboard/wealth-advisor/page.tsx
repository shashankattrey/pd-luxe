...
function RateTicker({
  ...
  return (
    <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] overflow-hidden mb-1 w-full">
...
export default function WealthAdvisorPage() {
  ...
  // --- Internal Helper Component for Action Cards ---
const RecommendationCard = ({ 
  ...
  const cfg = PRIORITY[rec.priority as RecommendationPriority];
  const invCfg = INV_TYPE[rec.investmentType as any] ?? INV_TYPE.sip;
  const InvIcon = invCfg.icon;
  const isExp = expanded === rec.id;
  const amtCount = [rec.monthlyAmount > 0, rec.lumpSum > 0, rec.taxSaving].filter(Boolean).length;
  ...
  return (
    ...
    >
      <div style={{ height: rec.priority === "critical" ? "3px" : "1.5px", background: `linear-gradient(90deg,${cfg.color},${cfg.color}44,transparent)` }} />
      <div className="p-4" style={{ background: `${cfg.color}06` }}>
        <div className="flex items-start gap-3">
          <span className="text-2xl shrink-0 mt-0.5">{rec.emoji}</span>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              <span className="text-[9px] font-bold px-2 py-0.5 rounded-full border" style={{ color: cfg.color, borderColor: `${cfg.color}35`, background: `${cfg.color}12` }}>{cfg.label}</span>
              <span className={cn("inline-flex items-center gap-1 text-[9px] font-bold px-1.5 py-0.5 rounded-full", invCfg.color, invCfg.bg)}>
                <InvIcon className="w-2.5 h-2.5" />{invCfg.label}
              </span>
            </div>
            <p className="text-sm font-bold leading-snug">{rec.title}</p>
          </div>
          <div className="shrink-0 mt-1">
             <div className="w-5 h-5 rounded-full border border-white/15 flex items-center justify-center">
                {isExp ? <ChevronUp className="w-2.5 h-2.5 opacity-40" /> : <ChevronDown className="w-2.5 h-2.5 opacity-40" />}
             </div>
          </div>
        </div>
      </div>
      <AnimatePresence>
        {isExp && (
          <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden border-t border-white/[0.06] p-4 space-y-4">
             <p className="text-xs text-muted-foreground/70">{rec.reasoning}</p>
             <div className="p-3 rounded-xl border border-amber-400/20 bg-amber-500/5">
                <p className="text-[10px] font-bold text-amber-400 uppercase mb-1">Action</p>
  ...
  // Change this line
const [viewMode, setViewMode] = useState<"cards" | "report" | "score">("cards");
  ...
  if (step === 0)
    return (
      <div className="w-full max-w-4xl mx-auto space-y-20 relative min-w-0 overflow-hidden px-4">
        {/* Ambient glows */}
        <div
  ...
  return (
    <div className="w-full max-w-lg mx-auto space-y-6 relative">
      ...
      >
      {(["cards", "report", "score"] as const).map((mode) => (
  <button
    key={mode}
    onClick={() => setViewMode(mode)}
    // Added 'relative' so the motion.div can position itself absolutely inside
    className="relative flex-1 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all"
    style={{ color: viewMode === mode ? "#000" : "rgba(255,255,255,0.3)" }}
  >
    ...
    {/* THE CONTENT (Wrapped in z-10 so it stays above the slider) */}
    <span className="relative z-10 flex items-center justify-center gap-2">
     ...
     {/* 1. Calculate Health Score for the Gauge */}
{(() => {
  const healthScore = (() => {
    let s = 0;
    ...
    const emMonths = (profile.emergencyFund || 0) / monthlyExp;
    // 40pts for 6mo buffer, 30pts for Insurance, 30pts for Goal Feasibility
    s += Math.min(emMonths * 6.6, 40); 
    if (profile.insuranceTerm) s += 15;
    if (profile.insuranceHealth) s += 15;
    // Calculate the surplus manually from the profile fields
const surplus = (profile.monthlyIncome || 0) - (profile.monthlyExpenses || 0);
const feasibility = assessGoalFeasibility(profile, surplus);
    if (feasibility.status === "achievable") s += 30;
    else if (feasibility.status === "stretch") s += 15;
    return Math.round(s);
  })();
    ...
    <AnimatePresence mode="wait">
      ...
      {viewMode === "cards" && (
        <motion.div 
          ...
          exit={{ opacity: 0, y: -10 }}
        >
          {/* Keep your existing GoalProgressCard, StatBox grid, and rec.map logic here */}
          {summary && <GoalProgressCard summary={summary} />}
          {/* ... all your existing "cards" code ... */}
          <div className="space-y-3">
            {visibleRecs.map((rec, index) => (
               <RecommendationCard 
               key={rec.id} 
               rec={rec} 
               i={index} // or whatever your map index variable is named
               expanded={expanded} 
               setExpanded={setExpanded} 
               funds={funds} 
               loading={loading} 
             />// Assuming you refactored to a component or kept the mapping
            ))}
          </div>
        </motion.div>
      )}
      ...
      {/* TAB 3: WEALTH SCORE (The Waterfall Effect) */}
      {viewMode === "score" && (
        <motion.div 
          key="score" 
          ...
          animate={{ opacity: 1, x: 0 }} 
          exit={{ opacity: 0, x: -20 }}
          className="p-8 md:p-12 rounded-[3rem] bg-white/[0.02] border border-white/10 relative overflow-hidden"
        >
          {/* Background Glow */}
          ...
          <div className="flex flex-col md:flex-row items-center gap-12 relative z-10">
            ...
            <div className="flex flex-col items-center gap-4 min-w-[160px]">
              <WealthScore score={healthScore} />
              <div className="text-center">
                <p className="text-[10px] font-bold text-white/40 uppercase tracking-[0.2em]">Wealth Health Index</p>
                <p className="text-[9px] text-emerald-500/50 mt-1 uppercase font-medium">Live Audit</p>
              </div>
            </div>

            {/* WATERFALL */}
            <div className="flex-1 w-full border-t md:border-t-0 md:border-l border-white/5 pt-10 md:pt-0 md:pl-12">
              <div className="flex items-center justify-between mb-10">
                <div>
                  <h3 className="text-[10px] font-bold text-white/40 uppercase tracking-[0.3em]">Priority Path</h3>
                  <p className="text-[9px] text-muted-foreground/20 mt-1 uppercase">Foundation Check</p>
                </div>
                <Sparkles className="w-4 h-4 text-emerald-400/20" />
              </div>
              <WaterfallProgress profile={profile} />
            </div>
          </div>
        </motion.div>
      )}
